...
var GoogleAnalytics2 = function () {
  function GoogleAnalytics2(odl, data, config) {
    ...
    if (!_opendatalayer.window.ga) {
      (function (i, s, o, g, r, a, m) {
        i['GoogleAnalyticsObject'] = r;i[r] = i[r] || function () {
          (i[r].q = i[r].q || []).push(arguments);
        }, i[r].l = 1 * new Date();a = s.createElement(o), m = s.getElementsByTagName(o)[0];a.async = 1;a.src = g;m.parentNode.insertBefore(a, m);
      })(_opendatalayer.window, document, 'script', '//www.google-analytics.com/analytics.js', 'ga');
    }
  ...
  babelHelpers.createClass(GoogleAnalytics2, [{
    ...
    value: function initGA(data, config) {
      ...
      _opendatalayer.window.ga('create', uaId, gaConfig);
      _opendatalayer.window.ga('galeria.set', 'anonymizeIp', true);
      _opendatalayer.window.ga('galeria.require', 'displayfeatures');
      ...
      if (data.page.type === 'productdetail' || data.page.type === 'checkout-cart' || data.page.type === 'checkout-login' || data.page.type === 'checkout-lastCheck' || data.page.type === 'checkout-confirmation') {
        _opendatalayer.window.ga('galeria.require', 'ec');
      }
      ...
      switch (data.page.type) {
        ...
        case 'productdetail':
          ...
          _opendatalayer.window.ga('galeria.ec:setAction', 'detail');
          break;
        case 'checkout-cart':
          for (var i = 0; i < data.cart.products.length; i++) {
            this.ecAddProduct(data.cart.products[i]);
          }
          _opendatalayer.window.ga('galeria.ec:setAction', 'checkout', { step: 1 });
          break;
        case 'checkout-login':
          _opendatalayer.window.ga('galeria.ec:setAction', 'checkout', { step: 2 });
          break;
        case 'checkout-lastCheck':
          _opendatalayer.window.ga('galeria.ec:setAction', 'checkout', { step: 3 });
          break;
        case 'checkout-confirmation':
          {
            ...
            _opendatalayer.window.ga('galeria.ec:setAction', 'purchase', {
              id: o.id,
              affiliation: '',
      ...
      this.trackDimension('dimension2', escape(_opendatalayer.window.navigator.userAgent), true);

      // handle campaign tracking
      this.trackCampaign();

      // send pageview
      _opendatalayer.window.ga('galeria.send', 'pageview');
    }
    ...
    value: function handleEvent(name, data) {
      if (name === 'addtocart') {
        ...
        this.ecAddProduct(data.product);
        _opendatalayer.window.ga('galeria.ec:setAction', 'add');
        return _opendatalayer.window.ga('galeria.send', 'event', 'UX', 'click', 'add to cart');
      }
      return false;
    }
    ...
    value: function trackDimension(name, value) {
      ...
      return _opendatalayer.window.ga('galeria.set', d);
    }
    ...
    value: function trackCampaign() {
      // get URL params
      var query = {};
      var q = _opendatalayer.window.location.search.substring(1);
      ...
      if (medium && __guard__(campaignData, function (x1) {
        ...
        return _opendatalayer.window.ga('galeria.set', {
    ...
    value: function ecAddProduct(product) {
      ...
      // see https://jira.gkh-setu.de/browse/BSNA-716 for details
      return _opendatalayer.window.ga('galeria.ec:addProduct', {
        id: product.ean,
        name: product.name,
